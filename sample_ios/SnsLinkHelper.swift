...
struct ScrapInfo {
    ...
    func toJsonString() -> String! {
        ...
        return SnsLinkHelper.convertJsonString(dictionary as AnyObject)
    }
}

class SnsLinkHelper: NSObject {
    ...
    //BAND
    class func canOpenBAND() -> Bool {
        if let url = URL(string: "bandapp://") {
            return UIApplication.shared.canOpenURL(url)
        }
        return false
    }
    class func makeBANDLink(_ text: String, _ route: String) -> String! {
        var parameters: [String: String] = [String: String]()
        parameters["text"] = text
        parameters["route"] = route
        let parameterString = self.HTTPArgumentsStringForParameters(parameters)
        return "bandapp://create/post?\(parameterString ?? "")"
    }
    
    //LINE
    class func canOpenLINE() -> Bool {
        if let url = URL(string: "line://") {
            return UIApplication.shared.canOpenURL(url)
        }
        return false
    }
    class func makeLINELink(_ text: String) -> String! {
        var parameters: [String: String] = [String: String]()
        parameters["text_message"] = text
        var parameterString = self.HTTPArgumentsStringForParameters(parameters)
        parameterString = parameterString?.replacingOccurrences(of: "text_message=", with: "")
        return "line://msg/text/?\(parameterString ?? "")"
    }
    
    
    //Common
    class func openiTunes(_ urlString: String) {
    ...
    class func openSNSLink(_ urlString: String) -> Bool {
        if urlString.isEmpty {
            print("SNS URL is empty.");
            return false
        }
